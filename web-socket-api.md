...
- [Request security](#request-security)
  ...
  - [SIGNED request example](#signed-request-example)
...
See [Error codes](errors.md) for a list of error codes and messages.
...
* Optional `returnRateLimits` boolean parameter in connection URL.
  ...
  ```
  wss://ws-api.binance.us/ws-api/v3?returnRateLimits=false
  ```
...
* Secure methods require a valid API key to be specified and authenticated.
  ...
  * **Both API key and secret key are sensitive.** Never share them with anyone.
    If you notice unusual activity in your account, immediately revoke all the keys and contact Binance.US support.
...
* The signature is computed using HMAC-SHA-256 algorithm.
  See [computation example](#signed-request-example) below.
...
```javascript
{
  ...
  "result": {
    ...
    "type": "LIMIT",
    "side": "SELL",
    "stopPrice": "0.00000000",          // always present, zero if order type does not use stopPricer
